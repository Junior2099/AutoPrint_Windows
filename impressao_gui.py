...
def listar_arquivos():
    """Lista os arquivos PDF, imagens e Word na pasta Downloads"""
    extensoes = [".pdf", ".jpg", ".jpeg", ".png", ".gif", ".bmp", ".doc", ".docx", ".rtf"]
...
def atualizar_lista():
    ...
    for i, arquivo in enumerate(arquivos):
        ...
        if arquivo.lower().endswith('.pdf'):
            tipo = "📄 PDF"
        elif arquivo.lower().endswith(('.doc', '.docx', '.rtf')):
            tipo = "📝 Word"
        else:
            tipo = "🖼️ Imagem"
    ...
    status_text = f"✅ {total_arquivos} arquivos encontrados"
    if PDF_SUPPORT:
        status_text += " | PyPDF2 instalado"
    else:
        status_text += " | ❌ PyPDF2 não instalado"
    
    if WORD_SUPPORT:
        status_text += " | pywin32 instalado"
    else:
        status_text += " | ❌ pywin32 não instalado"
...
def get_word_page_count(file_path):
    """Obtém o número total de páginas em um documento do Word"""
    if not WORD_SUPPORT:
        return 0
    ...
    try:
        ...
        messagebox.showerror("❌ Erro", f"Erro ao obter número de páginas do Word:\n{e}")
        return 0

def print_specific_word_pages(file_path, pages):
    """Imprime páginas específicas de um documento Word"""
    if not WORD_SUPPORT:
        return False
    ...
    try:
        ...
        word.ActivePrinter = word.Application.ActivePrinter
        doc.PrintOut(Range=0, Pages=pages_str)
        
        doc.Close(False)
        word.Quit()
        return True
    except Exception as e:
        messagebox.showerror("❌ Erro", f"Erro ao imprimir páginas específicas do Word:\n{e}")
        return False

def imprimir_word(arquivo):
    """Pergunta antes de imprimir e executa a impressão de arquivos Word"""
    caminho_arquivo = os.path.join(pasta_downloads, arquivo)
    
    if not WORD_SUPPORT:
        resposta = messagebox.showinfo("⚠️ Informação", 
                              "A seleção de páginas para documentos Word requer o módulo pywin32.\n\n"
                              "Execute 'pip install pywin32' e reinicie o aplicativo para habilitar esta funcionalidade.\n\n"
                              "Deseja imprimir o documento inteiro?")
        try:
            subprocess.run(["powershell", f'Start-Process -FilePath "{caminho_arquivo}" -Verb Print'], check=True, shell=True)
            fechar_com_mensagem(f"Impressão enviada para {arquivo}!")
        except subprocess.CalledProcessError as e:
            messagebox.showerror("❌ Erro", f"Erro ao imprimir documento Word:\n{e}")
        return
    ...
    if resposta == "yes":
        ...
        try:
            total_pages = get_word_page_count(caminho_arquivo)
            if total_pages > 0:
                selecionar_paginas_word(caminho_arquivo, arquivo, total_pages)
            else:
                messagebox.showerror("❌ Erro", "Não foi possível determinar o número de páginas do documento.")
        except Exception as e:
            messagebox.showerror("❌ Erro", f"Erro ao processar o documento Word:\n{e}")

def selecionar_paginas_word(caminho_arquivo, nome_arquivo, total_pages):
    """Cria uma janela para selecionar páginas de documentos Word"""
    ...
    def processar_impressao_word():
        ...
        try:
            if print_specific_word_pages(caminho_arquivo, page_numbers):
    ...
    btn_imprimir = tk.Button(frame_botoes, text="🖨️ Imprimir", command=processar_impressao_word, 
                          font=("Arial", 11, "bold"), bg="#008CBA", fg="white", padx=15, pady=5)
    btn_imprimir.pack(side=tk.LEFT, padx=10)
    janela_paginas.bind('<Return>', lambda event: processar_impressao_word())
...
def imprimir_pdf():
    ...
    if not PDF_SUPPORT:
        messagebox.showinfo("⚠️ Informação", 
                          "A seleção de páginas para PDFs requer o módulo PyPDF2.\n\n"
                          "Execute 'pip install PyPDF2' e reinicie o aplicativo para habilitar esta funcionalidade.\n\n"
                          "O documento inteiro será impresso.")
        try:
            subprocess.run(["powershell", f'Start-Process -FilePath "{caminho_arquivo}" -Verb Print'], check=True, shell=True)
            fechar_com_mensagem(f"Impressão enviada para {arquivo}!")
        except subprocess.CalledProcessError as e:
...
janela = tk.Tk()
janela.title("🖨️ Impressão de Arquivos PDF, Word e Imagens")
...
titulo = tk.Label(frame_titulo, text="Impressão de Arquivos PDF, Word e Imagens", font=("Arial", 16, "bold"), bg="#f0f0f0")
titulo.pack()
